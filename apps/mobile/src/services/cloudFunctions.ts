 ...
 * Using modular API for region-specific functions
 */
...
// Get Cloud Functions instance for asia-northeast1 region (React Native Firebase v22)
const functionsRegion = getFunctions(undefined, "asia-northeast1");
...
export const getTodayTask = async (): Promise<GetTodayTaskResponse> => {
  try {
    ...
    const callable = httpsCallable(functionsRegion, "getTodayTask");
...
export const completeTask = async (historyId: string): Promise<CompleteTaskResponse> => {
  try {
    ...
    const callable = httpsCallable(functionsRegion, "completeTask");
