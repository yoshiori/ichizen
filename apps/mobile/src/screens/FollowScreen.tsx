...
export const FollowScreen: React.FC = () => {
  ...
  const loadFollowing = async () => {
    ...
    try {
      ...
      Alert.alert(
        t('follow.error', 'エラー'),
        t('follow.loadError', 'フォロー一覧の読み込みに失敗しました')
      );
    } finally {
      setRefreshing(false);
    }
  ...
  const handleFollow = async () => {
    ...
    if (followingId === user.id) {
      Alert.alert(
        t('follow.error', 'エラー'),
        t('follow.selfFollowError', '自分をフォローすることはできません')
      );
      return;
    }
    ...
    try {
      ...
      const targetUser = await getUser(followingId);
      if (!targetUser) {
        Alert.alert(
          t('follow.error', 'エラー'),
          t('follow.userNotFound', 'ユーザーが見つかりません')
        );
        return;
      }

      // Check if already following
      const alreadyFollowing = await isFollowing(user.id, followingId);
      if (alreadyFollowing) {
        Alert.alert(
          t('follow.error', 'エラー'),
          t('follow.alreadyFollowing', 'すでにフォローしています')
        );
        return;
      }
      ...
      Alert.alert(
        t('follow.success', '成功'),
        t('follow.followSuccess', 'ユーザーをフォローしました')
      );
      ...
      Alert.alert(
        t('follow.error', 'エラー'),
        t('follow.followError', 'フォローに失敗しました')
      );
    } finally {
      setLoading(false);
    }
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                t('follow.error', 'エラー'),
                t('follow.unfollowError', 'フォロー解除に失敗しました')
              );
            } finally {
              setLoading(false);
            }
    ...
    return (
      <View style={styles.container}>
        <Text style={styles.errorText}>
          {t('follow.authRequired', 'ログインが必要です')}
        </Text>
      </View>
    );
  }
    ...
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>
            {t('follow.followUser', 'ユーザーをフォロー')}
          </Text>
          <TextInput
            style={styles.input}
            placeholder={t('follow.enterUserId', 'ユーザーIDを入力')}
            value={followingId}
