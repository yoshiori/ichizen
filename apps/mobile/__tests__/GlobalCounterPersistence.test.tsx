...
import {subscribeToGlobalCounter, unsubscribeFromGlobalCounter} from "../src/services/globalCounterSubscription";
...
describe("GlobalCounter Firestore Persistence", () => {
  ...
  it("should subscribe to Firestore updates on mount", async () => {
    ...
    const {getByTestId} = render(<GlobalCounter subscribeToUpdates onCounterUpdate={mockCallback} />);
  ...
  it("should handle real-time Firestore updates", async () => {
    ...
    const {getByTestId} = render(<GlobalCounter subscribeToUpdates animateChanges />);
